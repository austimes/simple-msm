...
test('resolver keeps crude steel as a direct pathway bundle by default', () => {
  ...
  assert.equal(
    resolved.activeRepresentationByRole.make_crude_steel,
    'make_crude_steel__pathway_bundle',
  );
  assert.deepEqual(
    resolved.activeMethodIdsByRole.make_crude_steel,
    [
  ...
  assert.equal(resolved.activeRoleIds.includes('make_direct_reduced_iron'), false);
  ...
  assert.equal(activeRows.some((row) => row.role_id === 'make_direct_reduced_iron'), false);
});
...
test('resolver activates crude steel child roles under the H2 DRI decomposition', () => {
  ...
  assert.equal(
    resolved.activeRepresentationByRole.make_crude_steel,
    'make_crude_steel__h2_dri_decomposition',
  );
  assert.deepEqual(resolved.activeMethodIdsByRole.make_crude_steel, []);
  assert.deepEqual(
    resolved.roles
      .find((role) => role.roleId === 'make_crude_steel')
      ?.activeChildRoleIds,
    [
  ...
  assert.deepEqual(
    resolved.activeMethodIdsByRole.make_direct_reduced_iron,
    [
  ...
  assert.deepEqual(
    resolved.activeMethodIdsByRole.melt_and_refine_dri_into_crude_steel,
    [
      'steel__dri_melt_refine__eaf_finishing',
      'steel__dri_melt_refine__electric_smelter',
    ],
  );
  assert.equal(resolved.activeMethodIdsByRole.make_crude_steel.includes('steel__crude_steel__h2_dri_electric'), false);
});
...
test('buildSolveRequest filters rows through the selected role representation structure', () => {
  ...
  assert.equal(directRequest.rows.some((row) => row.outputId === 'make_direct_reduced_iron'), false);
  ...
  assert.ok(decomposedRequest.rows.some((row) => row.outputId === 'make_direct_reduced_iron'));
  assert.ok(decomposedRequest.rows.some((row) => row.outputId === 'melt_and_refine_dri_into_crude_steel'));
...
test('resolver rejects double coverage, inactive required roles, and missing representation selections', () => {
  ...
  assert.throws(
    ...
    /Required child role "make_direct_reduced_iron" is inactive/,
  );
  ...
  assert.throws(
    () => resolveActiveRoleStructure(
      ...
      {
        representation_by_role: {
          make_direct_reduced_iron: 'make_crude_steel__pathway_bundle',
        },
      },
    ),
    /belongs to role "make_crude_steel", not "make_direct_reduced_iron"/,
  );
    ...
    () => resolveActiveRoleStructure(
      {
        ...
        representations: pkg.representations.filter((representation) =>
          representation.role_id !== 'make_crude_steel',
        ),
      },
      {},
    ),
