...
- `make_crude_steel__pathway_bundle` is the default aggregate pathway bundle and exposes the current BF-BOF, scrap EAF, CCS-influenced BF-BOF, and aggregate H2 DRI-electric methods.
- `make_crude_steel__h2_dri_decomposition` is an optional role decomposition. It deactivates the parent direct methods and activates the required child roles `make_non_h2_dri_crude_steel`, `make_direct_reduced_iron`, and `melt_and_refine_dri_into_crude_steel`.
