...
- `make_crude_steel__pathway_bundle` must remain the default aggregate representation.
- `make_crude_steel__h2_dri_decomposition` must remain a non-default `role_decomposition` representation with no direct methods on the parent role.
- The decomposition must activate `make_non_h2_dri_crude_steel`, `make_direct_reduced_iron`, and `melt_and_refine_dri_into_crude_steel` as required child roles.
