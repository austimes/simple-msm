...
- The high-temperature packages must share the same role-local non-stacking group.
- Any accepted high-temperature package should remain a small wedge relative to route-switch uncertainty and should not target `__electrified` or `__low_carbon_fuels` methods.
- The fossil incumbent row should be rebaselined so the explicit autonomous track, not `method_years.csv`, carries the accepted background drift.
