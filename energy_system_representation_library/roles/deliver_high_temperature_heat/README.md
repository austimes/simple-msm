...
This role covers `high_temperature_heat` in sector `generic_industrial_heat` / subsector `high_temperature_heat` for region `AUS`. The canonical balance type is `required_service`, with output measured in `GJ_useful_heat`. Generic industrial heat is split into three useful-heat service bands:

## Output/service definition

- Role id: `high_temperature_heat`
