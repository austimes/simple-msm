# Melt and refine DRI into crude steel
...
This role is activated by `make_crude_steel__h2_dri_decomposition` and consumes `direct_reduced_iron` from `make_direct_reduced_iron`. The current data is a pilot split from aggregate crude-steel route archetypes, not a plant-level engineering model.
