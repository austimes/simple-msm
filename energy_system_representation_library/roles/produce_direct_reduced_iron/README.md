...
This role is activated by `produce_crude_steel__h2_dri_decomposition` and supplies `direct_reduced_iron` to `melt_refine_dri_crude_steel`. The current data is a pilot split from aggregate crude-steel route archetypes, not a plant-level engineering model.
