...
This family covers `low_temperature_heat` in sector `generic_industrial_heat` / subsector `low_temperature_heat` for region `AUS`. The canonical output role is `required_service`, with output measured in `GJ_useful_heat`. Generic industrial heat is split into three useful-heat service bands:

## Output/service definition

- Family id: `low_temperature_heat`
